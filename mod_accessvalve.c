...
/**
 * this is a accessvalve for a lighttpd plugin
 *
 * just replaces every occurance of 'accessvalve' by your plugin name
 *
 * e.g. in vim:
 *
 *   :%s/accessvalve/myhandler/
 *
 */
...
SETDEFAULTS_FUNC(mod_accessvalve_set_defaults) {
	...
	config_values_t cv[] = {
		{ "accessvalve.array",             NULL, T_CONFIG_ARRAY, T_CONFIG_SCOPE_CONNECTION },       /* 0 */
...
static int mod_accessvalve_patch_connection(server *srv, connection *con, plugin_data *p) {
	...
	for (i = 1; i < srv->config_context->used; i++) {
		...
		for (j = 0; j < dc->value->used; j++) {
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("accessvalve.array"))) {
				PATCH(match);
			}
		}
	}
